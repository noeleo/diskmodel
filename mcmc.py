...
#Step 1:  Initialize the Chain.  These values are from a previous best-fit
innerRad1 = 72.
outerRad1 = innerRad1*1.05  #Set equal to innerRad x 1.05
grainSize1 = -1.5  #Will be raised to the 10th power
diskMass1 = -4.2  #Will be raised to the 10th power
powerLaw1 = 0.5  #Currently fixed
grainEfficiency1 = 0.38
beltMass1 = -5.9 #Will be raised to the 10th power
...
disk = Disk(innerRad1, outerRad1, 10**grainSize1, 10**diskMass1, powerLaw1, grainEfficiency1, 10**beltMass1)
...
while count < stop:    
    ...
    if choice == 1:
        ...
        innerRad2=innerRad1
    outerRad2 = innerRad2*1.05 #Currently fixed with respect to inner radius.
    if choice == 2:
    ...
    disk.changeParameters(innerRad1, outerRad1, 10**grainSize1, 10**diskMass1, powerLaw1, grainEfficiency1, 10**beltMass1)
    sedchi1 = disk.computeChiSquared()
    vischi1 = vis.computeChiSquared(disk)
    chi1 = sedchi1 + vischi1
    
    disk.changeParameters(innerRad2, outerRad2, 10**grainSize2, 10**diskMass2, powerLaw2, grainEfficiency2, 10**beltMass2)
    ...
    outerRadsteps.append(outerRad1)
    ...
    count = count + 1
    print 'Step =', count
    print 'IR =', innerRad1, 'log(GS) =', grainSize1, 'log(DM) =', diskMass1, 'GE =', grainEfficiency1, 'log(BM) =', beltMass1, 'chi1 =', chi1, 'chi2 =', chi2, "accept =", accept
    if (len(sys.argv) == 2):
        save.write(str(count)+' '+str(innerRad1)+' '+str(outerRad1)+' '+str(grainSize1)+' '+str(diskMass1)+' '+str(powerLaw1)+' '+str(grainEfficiency1)+' '+str(beltMass1)+' '+str(chi1)+' '+str(chi2)+' '+str(accept)+'\n')
